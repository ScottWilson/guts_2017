...
def random_occurrences(intent, session):
    session_attributes = {}

    # select random character from alphabet and number of occurrences
    random_char = char(random.randint(65,90))
    random_occur = random.randint(1,10)
    answer = random_occur
    session_attributes['answer'] = answer

    # build question
    speech_output = random_char * random_occur + ". How many times did I say " + random_char + "?"
    reprompt_text = "HOWDY PARDTNER"
...
def random_occurrences_answer(intent, session):
    # Return answer to random occurrences question

    session_attributes = session['attributes']
    speech_output = "The answer is: " + session_attributes['answer']
    reprompt_text = "HOWDY PARTDNER"
    should_end_session = False
